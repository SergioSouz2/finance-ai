...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  // simulando o uso do OpenAI por nao tenho pago
  ...
  // PEGAR AS TRANSACOES DOO MES RECEBIDO
  ...
  // MANDA  AS  TRANSACOES PARA  O  CHAT GPT E PEDIR PARA ELE GERAR UM RELATORIO COM INSIGNTS]
  const content = `Gere um relatorio com insights sobre minhas financas, com dicas e orientacoes de como melhorar minha vida financeira, as transacoes estao divididas por ponto e virgula. a estrutura de cada uma e {DATA}-{TIPO}-{VALOR}-CATEGORIA. sao elas :
  ${transactions
    .map(
      (transaction) =>
    ...
    messages: [
      {
        role: "system",
        content:
          "Você é um assistente financeiro. Você deve gerar um relatório com insights sobre as finanças do usuário, com dicas e orientações de como melhorar sua vida financeira.",
      },
      {
        role: "user",
        content,
  ...
  // PEGAR O RELATORIO GERADO PELO CHAT GPT E RETORNAR PARA O USUARIO
